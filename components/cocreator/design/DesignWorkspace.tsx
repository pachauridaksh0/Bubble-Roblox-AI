...
const ImageModal: React.FC<{ src: string; onClose: () => void }> = ({ src, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ type: 'spring', stiffness: 260, damping: 20 }} className="relative w-full h-full bg-transparent flex items-center justify-center overflow-hidden" onWheel={handleWheel}>
            ...
            {generatedImages.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {generatedImages.map((imgSrc, index) => (
                        <motion.div key={index} layoutId={`image-${index}`} className="aspect-square bg-bg-secondary rounded-lg overflow-hidden cursor-pointer group" onClick={() => setSelectedImage(imgSrc)}>
