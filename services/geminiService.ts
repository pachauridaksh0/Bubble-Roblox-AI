...
export const generateImage = async (prompt: string, apiKey: string, model: ImageModel = 'nano_banana'): Promise<string> => {
    const ai = new GoogleGenAI({ apiKey });
    try {
        // Use the high-quality, slower model for Imagen 2 and 3 selections.
        if (model === 'imagen_2' || model === 'imagen_3') {
            ...
            throw new Error("Imagen model did not return an image. The response was empty.");
        } 
        
        // Use the fast, lower-quality model for Nano Banana.
        ...
        // Fallback or error for any unsupported model selection.
        throw new Error(`Unsupported or unrecognized image model specified: '${model}'.`);
